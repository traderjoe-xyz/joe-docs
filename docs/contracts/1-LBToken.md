...
_Mapping from token id to account balances_

### _spenderApprovals

```solidity
mapping(address => mapping(address => bool)) _spenderApprovals
```

_Mapping from account to spender approvals_
...
_Mapping from token id to total supplies_

### _userIds

```solidity
mapping(address => struct EnumerableSet.UintSet) _userIds
```

_Mapping from account to set of ids, where user currently have a non-zero balance_
