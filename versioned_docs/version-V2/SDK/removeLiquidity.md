...
Use the [subgraph](/versioned_docs/version-V2/subgraphs/avalanche.md) to fetch your positions. You need all the `binId`s where you have liquidity and the amount of `liquidity` in each bin. Below is an example of a GraphQL query to the subgraph's `LiquidityPositions` entity.
```graphql
...
Note that `removeLiquidityParams` will look different for the `removeLiquidityAVAX` method. Please refer to this [link](/versioned_docs/version-V2/guides/manage-a-liquidity-position.md#removing-liquidity) for specific details.
